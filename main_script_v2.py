...
def target_gene_expression_analysis(mirna2age, mirna2disease,mirna2family,gene2age):
		...
		gen1 = [sum(mirna_in_hamming_2_exp[a]) for a in mir_in_fam if mirna2age[a] < 100.0 ]
		gen2 = [sum(mirna_in_hamming_2_exp[a]) for a in mir_not_in_fam if mirna2age[a] < 100.0] 
		...
		for fam in mirna2family:
			...
			for mir in mirna2family[fam]:
				for secmir in mirna2family[fam]:
					...
					lst_ham.append(hamming(sum(mirna_in_hamming_2_exp[mir]), sum(mirna_in_hamming_2_exp[secmir]), normalized=True))
