...
def age_proteins_dollo(species_tree, species2prots, family2prots, 
    ...
    for family in family2prots:
        ...
        # method to try both best method - We could look at this approach again if need be, but right now I'm abandoning it

        # lst_of_ans = []

        # for taxon in taxon_labels:
        #     initial_mrca = species_tree.mrca(taxon_labels=[taxon,])
        #     if initial_mrca.label == None:
        #         lst_of_ans.append(initial_mrca.parent_node.label)
        #     else:
        #         lst_of_ans.append(initial_mrca.label)
