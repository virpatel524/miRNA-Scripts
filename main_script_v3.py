...
def heatmap_analysis(mirna2age, mirna2disease, mirna2family, gene2age):
	...
	mirnas_in_fam = flatten(mirna2family_edited.values())
	mirnas_notinfam = [a for a in mirna2age.keys() if a not in flatten(mirna2family_edited.values())]
	...
	for alpha in round_robyn_target.index:
		for beta in round_robyn_target.index:
			if alpha == beta: continue
			if alpha in flatten(mirna2family_edited.values()) and beta in flatten(mirna2family_edited.values()):
	...
	plt.savefig('figures/mir_fam_tar_hamming_violin.pdf',bbox_inches='tight')
